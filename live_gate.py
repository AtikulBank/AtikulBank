...
def try_connect_and_login(host, port, sender_comp_id, target_comp_id, sender_sub_id, password):
    ...
    try:
        ...
        logon_msg = encoder.create_logon(password)
        print(f"  [LOGON] Sending logon message...")
...
def main():
    ...
    print("  LIVE cTrader FIX Connection Mode")
    print("  NO SIMULATED FALLBACK - Requires valid FIX connection")
    print("=" * 70)
    print()
    ...
    # Initialize Quantum Brain (only after successful connection)
    print("\n[QUANTUM BRAIN] Initializing 100+ mathematical filters...")
    ...
    # Main engine loop
    print()
    print(f"[ENGINE] Starting main engine loop with LIVE XAUUSD data")
                    ...
                    if result["type"] in ("market_data_snapshot", "market_data_incremental"):
                        ...
                        if abs(composite) > 0.15 and confidence > 0.3:
                            ...
                            try:
                                ...
                                print(f"  [ORDER] #{tick_count} {action} | Qty={quantity:.2f} | Price={order_price:.5f} | Signal={composite:+.3f}")
                            except Exception as e:
